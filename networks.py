...
def get_weight(shape, gain=np.sqrt(2), use_wscale=False, fan_in=None, name='weight'):
    if fan_in is None: fan_in = np.prod(shape[:-1])
    std = gain / np.sqrt(fan_in) # He init
    if use_wscale:
        wscale = tf.constant(np.float32(std), name='wscale')
        return tf.get_variable(name, shape=shape, initializer=tf.initializers.random_normal()) * wscale
    else:
...
def apply_film(x, text_embed, weight_decay_film, **kwargs):
    ...
    with tf.variable_scope('FILM'):
        ...
        if len(x.shape) == 2:
            ...
            return x*tf.reshape(gamma, [-1, fmaps, 1, 1]) + tf.reshape(beta, [-1, fmaps, 1, 1])
