...
basis_lr = hparams['basis_lr']
train_kimg = hparams['train_kimg']
trans_kimg = hparams['trans_kimg']
...
desc += '-ssense';             dataset = EasyDict(tfrecord_dir='/data/tf_record_images_128')
...
desc += '-preset-v3-8gpus'; num_gpus = 8; sched.minibatch_base = 32;sched.minibatch_dict = {4: 2048, 8: 1024, 16: 512, 32: 256, 64: 128, 128:64, 256:32, 512:16, 1024:8}; sched.G_lrate_dict = {4: base_lr / basis_lr**0, 8: base_lr / 2**1, 16: base_lr / basis_lr**2, 32: base_lr / basis_lr**3, 64: base_lr / basis_lr**4, 128: base_lr / basis_lr**5, 256: base_lr / basis_lr**6, 512: base_lr / basis_lr**6, 1024: base_lr / basis_lr**7}; train.total_kimg = (trans_kimg + train_kimg) * 10; sched.lod_transition_kimg = trans_kimg; sched.lod_training_kimg = train_kimg

# Numerical precision (choose one).
# desc += '-fp32'; sched.max_minibatch_per_gpu = {256: 16, 512: 8, 1024: 4}
# desc += '-fp16'; G.dtype = 'float16'; D.dtype = 'float16'; G.pixelnorm_epsilon=1e-4; G_opt.use_loss_scaling = True; D_opt.use_loss_scaling = True; sched.max_minibatch_per_gpu = {512: 16, 1024: 8}
# desc += '-fp16'; G.dtype = 'float16'; D.dtype = 'float16'; D.epsilon=1e-4; G.pixelnorm_epsilon=1e-4; G_opt.use_loss_scaling = True; D_opt.use_loss_scaling = True; sched.max_minibatch_per_gpu = {512: 16, 1024: 8}; sched.minibatch_dict = {4: 2048, 8: 1024, 16: 512, 32: 256, 64: 128, 128:64, 256:32, 512:16, 1024:8}; sched.G_lrate_dict = {4: base_lr / basis_lr**0, 8: base_lr / 2**1, 16: base_lr / basis_lr**2, 32: base_lr / basis_lr**3, 64: base_lr / basis_lr**4, 128: base_lr / basis_lr**5, 256: base_lr / basis_lr**6, 512: base_lr / basis_lr**6, 1024: base_lr / basis_lr**7}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); sched.lod_transition_kimg = 1200; sched.lod_training_kimg = 1200
...
# train = EasyDict(func='util_scripts.generate_fake_images', run_id=2, num_pngs=50000); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
# train = EasyDict(func='util_scripts.generate_fake_images', run_id=2, grid_size=[15,8], num_pngs=10, image_shrink=4); num_gpus = 1; desc = 'fake-grids-' + str(train.run_id)
# train = EasyDict(func='util_scripts.generate_interpolation_video', run_id=2, grid_size=[1,1], duration_sec=60.0, smoothing_sec=1.0); num_gpus = 1; desc = 'interpolation-video-' + str(train.run_id)
# train = EasyDict(func='util_scripts.generate_training_video', run_id=2, duration_sec=20.0); num_gpus = 1; desc = 'training-video-' + str(train.run_id)
